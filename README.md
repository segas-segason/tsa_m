...
```
Рассмотрим квартальные данные по ВВП США с 1990 Q1 по н.в. (ряд gdp) и пусть y = log(gdp)

1. Задайте временной индекс
2. Визуализируйте ряд gdp_t, y_t, diff y_t, diff^2 y_t
3. Визуализируйте ряд diff^{1/2} y_t, diff^{3/2} y_t (ширина временного окна 5)
4. Постройте диаграмму рассеяния y_t vs y_{t-1}
5. Постройте диаграмму рассеяния diff y_t vs diff y_{t-1}
```
...
```
Рассмотрим месячные данные по M2 США с 1990-01-01 по н.в. (ряд m2) и пусть y = log(m2)

1. Задайте временной индекс
2. Визуализируйте ряд m2, y_t, diff y_t, diff^2 y_t
3. Визуализируйте ряд diff^{1/2} y_t, diff^{3/2} y_t (ширина временного окна 7)
4. Постройте диаграмму рассеяния y_t vs y_{t-1}
5. Постройте диаграмму рассеяния diff y_t vs diff y_{t-1}
```
...
```
Рассмотрим недельные данные по M2 США с 1990-01-01 по н.в.

1. Агрегируйте их в квартальные наблюдения (через усреднение)
2. Задайте временной индекс
3. Визуализируйте полученные наблюдения
```
...
```
Рассмотрим месячные данные краткосрочной (3-х мес, rate1) и долгосрочной (10-ти лет, rate2) ставкам для США с 1990-01-01 по н.в. как многомерный временной ряд rates.

1. Задайте временной индекс
2. Визуализируйте ряд rates двумя способами:
   - раздельные графики
   - общий график (два ряда на одном графике)
3. Визуализируйте ряд diff rates двумя способами
4. Визуализируйте ряд diff^2 rates двумя способами
5. Постройте гистограммы для rates, diff rates, diff^2 rates двумя способами
6. Постройте диаграмму рассеяния rate1 vs rate2
7. Постройте диаграмму рассеяния diff rate1 vs diff rate2
```
...
- краткосрочная (3-х мес) ставка
- долгосрочная (10-ти лет) ставка
- логарифм денежной массы M2
с 2000-01-01 по н.в. как многомерный временной ряд.

1. Задайте временной индекс
2. Визуализируйте многомерный ряд
3. Визуализируйте первую и вторую разность
```
...
1. Сформируйте месячный временной ряд из цены закрытия на последний день каждого месяца
2. Задайте для него временной индекс
3. Визуализируйте ряд
4. Визуализируйте первую и вторую логарифмические разности
```
...
```
Из finance.yahoo.com загрузите данные c 2005-01-01 по н.в. по ценам закрытия S&P500, Apple, Google

1. Сформируйте многомерный ряд из цен закрытия на последний день каждого месяца
2. Визуализируйте многомерный ряд
3. Визуализируйте первую и вторую логарифмические разности
```
...
1. Постройте графики ACF и PACF для y_t, diff y_t, diff^2 y_t
2. Значимы ли r(3), r_part(3) для diff y_t?
3. Вычислите {r(h)} от h=1 до 3 и {r_part(h)} от h=1 до 3 для diff y_t
```
...
1. Постройте графики ACF и PACF для y_t, diff y_t, diff^2 y_t
2. Значимы ли r(4), r_part(4) для diff y_t?
3. Вычислите {r(h)} от h=1 до 3 и {r_part(h)} от h=1 до 3 для diff y_t
```
...
```
Рассмотрим данные по S&P500 с 2000-01 по н.в. (ряд sp500) и пусть y = log(sp500)

1. Постройте графики ACF и PACF для y_t, diff y_t, diff^2 y_t
2. Значимы ли r(5), r_part(5) для diff y_t?
3. Вычислите {r(h)} от h=1 до 3 и {r_part(h)} от h=1 до 3 для diff y_t
```
...
φ(L)(1-L)^d y_t = θ(L) u_t, где u_t ~ WN(0, σ²)
для следующих многочленов:

№  d   φ(z)              θ(z)
1  0   1 - z + 0.25z²    1 + 0.8z
2  0   1 + 0.8z - 0.7z²  1 + 0.5z - 0.8z²
3  1   1 - 0.2z + 0.08z² 1 - 0.3z - 0.88z²
4  2   1 + 1.9z - 0.2z²  1 - 1.6z - 0.36z²

Для каждого случая:
1. Проверьте условия стационарности и обратимости
2. Запишите спецификацию модели без использования лагового оператора
```
...
```
Пусть y_t -- логарифм US GDP (квартальные данные) с 1995 по н.в.

1. Подгонка модели заданного порядка
   a) Подгоните модели:
      Модель          drift/trend  спецификация
      ARIMA(1,0,1)    +            y_t = α₀ + α₁t + φ y_{t-1} + u_t + θ u_{t-1}
      ARIMA(1,1,0)    +            diff y_t = α₀ + φ diff y_{t-1} + u_t
      ARIMA(1,1,1)    -            diff y_t = φ diff y_{t-1} + u_t + θ u_{t-1}
      ARIMA(1,2,0)    -            diff² y_t = φ diff² y_{t-1} + u_t
      и постройте прогноз на 10 периодов. Значим ли снос/тренд?
   b) Проведите диагностику каждой модели
   c) Проведите кросс-валидацию каждой модели. Какая предпочтительней?

2. Примените тесты единичного корня и найдите порядок интегрирования для y_t

3. Подгонка "оптимальной модели"
   a) Подгоните "оптимальную" модель ARIMA
   b) Проведите её диагностику
   c) Постройте прогноз на 10 периодов
```
...
```
Пусть y_t -- логарифм US M2 (месячные данные) с 1995 по н.в.
...
```
Пусть y_t -- логарифм US M2 (недельные данные) с 1995 по н.в.
...
```
Пусть y_t -- 10-летняя ставка (treasury securities with constant maturity, месячные данные) с 2000 по н.в.
...
```
Пусть y_t -- 10-летняя ставка (treasury securities with constant maturity, дневные данные) с 2010 по н.в.
...
```
Пусть y_t -- 3-месячная ставка (treasury bill, месячные данные) с 2000 по н.в.
...
```
Пусть y_t -- 3-месячная ставка (treasury bill, дневные данные) с 2010 по н.в.
...
```
Пусть y_t -- логарифм S&P500 (дневные данные) с 2010 по н.в.
...
1. ARIMA(0,1,1) без сноса и со сносом
2. ARIMA(0,1,2) без сноса и со сносом
3. ARIMA(1,1,0) без сноса и со сносом
4. ARIMA(2,1,0) без сноса и со сносом
5. ARIMA(0,2,0) без сноса и со сносом
6. ARIMA(1,2,0) без сноса и со сносом
7. ARIMA(0,2,1) без сноса и со сносом
```
...
```
Пусть y_t -- лог-доходность US M2 (недельные данные) с 1995 по н.в.

1. Подгоните модели AR-GARCH(p,o,q):
   Модель                λ
   AR(1)-GARCH(1,0,1)    2
   AR(1)-GARCH(1,0,1)    1
   AR(1)-GARCH(1,1,1)    2
...
2. Сравните модели по информационным критериям. Какая предпочтительней?
3. Проведите кросс-валидацию моделей. Какая предпочтительней?
...
```
Пусть ряд y_t -- первая разность 3-месячной ставки (treasury bill, дневные данные) с 2010 по н.в.
...
```
Пусть ряд y_t -- первая разность 10-летней ставки (treasury securities with constant maturity, дневные данные) с 2010 по н.в.
...
```
Пусть ряд y_t -- лог-доходность S&P500 (дневные данные) с 2010 по н.в.
...
```
Рассмотрим недельные данные с 2000 г по н.в. по следующим переменным:
- первая разность 3-месячной ставки (3-Month Treasury Bill)
- первая разность 6-месячной ставки (6-Month Treasury Bill)
- первая разность 10-летней ставки (Treasury Securities at 10-Year Constant Maturity)

Сформируйте многомерный ряд и визуализируйте его.

1. Фиксированный порядок
   a) Подгоните модели VAR(1), VAR(2), VAR(3)
   b) Постройте прогноз на 10 периодов по каждой модели
   c) Проведите кросс-валидацию моделей. Какая предпочтительней?

2. "Оптимизация" порядка
   a) Подгоните модель VAR "оптимального" порядка
   b) Проведите её диагностику
   c) Постройте прогноз на 5 периодов. Постройте FEVD
   d) Постройте IRF, используя исходное упорядочивание переменных
   e) Проведите тест Грэнджера на причинность
```
...
```
Рассмотрим дневные данные с 2000 г по н.в. по следующим переменным:
- первая разность 3-месячной ставки (3-Month Treasury Bill)
- первая разность 6-месячной ставки (6-Month Treasury Bill)
- первая разность 10-летней ставки (Treasury Securities at 10-Year Constant Maturity)
...
```
Рассмотрим месячные данные с 1995 г по н.в. по следующим переменным:
- первая разность 3-месячной ставки (3-Month Treasury Bill)
- первая разность 6-месячной ставки (6-Month Treasury Bill)
- первая разность 10-летней ставки (Treasury Securities at 10-Year Constant Maturity)
- лог-доходность US M2
...
```
Рассмотрим недельные данные с 2005 г по н.в. по следующим переменным:
- 3-месячная ставка (3-Month Treasury Bill)
- 6-месячная ставка (6-Month Treasury Bill)
- 1-летняя ставка (Treasury Securities at 1-Year Constant Maturity)
- 10-летняя ставка (Treasury Securities at 10-Year Constant Maturity)

Сформируйте многомерный ряд и визуализируйте его.

1. Найдите ранг коинтеграции
2. Оцените модель VECM "оптимального" порядка
3. Проведите её диагностику
4. Постройте прогноз на 5 периодов. Постройте FEVD
5. Постройте IRF, используя исходное упорядочивание переменных
6. Проведите тест Грэнджера на причинность
```
...
```
Рассмотрим месячные данные с 2005 г по н.в. по следующим переменным:
- 3-месячная ставка (3-Month Treasury Bill)
- 6-месячная ставка (6-Month Treasury Bill)
- 1-летняя ставка (Treasury Securities at 1-Year Constant Maturity)
- 10-летняя ставка (Treasury Securities at 10-Year Constant Maturity)
...
```
Рассмотрим VAR(1):
x_t = A x_{t-1} + u_t, где 
x_t = (x_t, y_t)', 
u_t = (u_t, v_t)',
u_t ~ WN(0, Σ), 
Σ = [[σ²_u, σ_uv], [σ_uv, σ²_v]] > 0

Проверить условие стационарности для следующих матриц A:
1. [[0.5, 1], [0, 0.3]]
2. [[0, 0.5], [-0.5, 0]]
3. [[1, 3], [0, 0.2]]
4. [[0, 1], [0, 1]]
...
4. { x_t = 3x_{t-1} - 7y_{t-1} + u_t
    y_t = x_{t-1} - 2.5y_{t-1} + v_t }

Для каждой модели:
1. Запишите в матричном виде
2. Проверьте условие стационарности
3. Определите, какие ряды коинтегрированы:
   - Если ряды коинтегрированы: запишите VECM модель и найдите коинтеграционные соотношения
   - Если ряды не коинтегрированы: запишите VAR-модель для дифференцированных рядов
```
...
x_t = A x_{t-1} + u_t, где 
x_t = (x_t, y_t, z_t)', 
u_t = (u_t, v_t, w_t)' ~ WN(0, Σ)

Проверьте условие стационарности для матриц A:
...
```
Рассмотрим VAR(2):
x_t = A1 x_{t-1} + A2 x_{t-2} + u_t, где 
x_t = (x_t, y_t)', 
u_t = (u_t, v_t)' ~ WN(0, Σ)
...
1. { x_t = 2x_{t-1} - x_{t-2} + u_t
    y_t = 1.5y_{t-1} - 0.5y_{t-2} + v_t }

2. { x_t = 1.5x_{t-1} + y_{t-1} - 0.5x_{t-2} - y_{t-2} + u_t
    y_t = -x_{t-1} - 0.5y_{t-1} + x_{t-2} + 1.5y_{t-2} + v_t }
